...
public class PieceDragAndDropListener implements MouseListener, MouseMotionListener {
    ...
    public void mouseReleased(MouseEvent e) {
        if (dragging) {
            boardPanel.postDrag();
            System.out.println("boardPanel.postDrag() successfully executed (PieceDragAndDropListener > mouseReleased)");
            // Dragging out of the board
            char calcFile = calculateFile(e);
            int calcRank = calculateRank(e);
            if (calcFile < 'a' || calcFile > 'h' || calcRank < 1 || calcRank > 8) {
                boardPanel.ignoreMoveRequest(originFile, originRank);
            }
            else {
            // Dragging out of the board
